...
describe("The test reducer", () => {
  ...
  it("should have default state", () => {
    expect(resetReducer(undefined, "NONE_EXISTENT_ACTION")).toEqual(
      initialState
    );
  });
  it("should change sending to true when action type is RESET_REQUEST_START", () => {
    expect(resetReducer(undefined, { type: "RESET_REQUEST_START" })).toEqual({
      ...initialState,
      sending: true
    });
  });
  it("should change sent to true when action type is RESET_REQUEST", () => {
    expect(resetReducer(undefined, { type: "RESET_REQUEST" })).toEqual({
      ...initialState,
      sent: true
    });
  });
  it("should change sent to false when action type is RESET_FAILED", () => {
    ...
    ).toEqual({ ...initialState, errors: "Failed To Reset", sent: false });
  });
});
